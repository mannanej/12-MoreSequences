...
def run_test_draw_points_on_circle():
    ...
    title = 'Test 4'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 50), 20)
...
def run_test_pizza():
    ...
    pizza(window, circle, 20, 'yellow', 20)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    title = 'POLYGON tests 3 and 4:'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 5
    polygon(window, circle, 5, 'green', 20)
    window.continue_on_mouse_click()

    circle = rg.Circle(rg.Point(300, 20), 10)
    circle.outline_thickness = 9
    polygon(window, circle, 12, 'purple', 4)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        line = rg.Line(point[k - 1], point[k])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 20, 5, 'black', 10)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(point)):
        line = rg.Line(point[k], point[(k + hops_to_next_point) % len(point)])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
